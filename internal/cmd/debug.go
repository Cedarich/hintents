...
func diffResults(res1, res2 *simulator.SimulationResponse, net1, net2 string) {
	...
	for i := 0; i < maxEvents; i++ {
		...
		// If interactive flag is set, show trace viewer
		if interactiveFlag {
			fmt.Println("\nLaunching interactive trace viewer...")
			fmt.Println("(Using mock trace data for demonstration)")
			
			// Create mock trace for now
			// TODO: Replace with actual simulation trace when simulator is integrated
			root := trace.CreateMockTrace()
			
			if err := trace.RunViewer(root); err != nil {
				return fmt.Errorf("failed to run trace viewer: %w", err)
			}
		}
...
func init() {
	...
	debugCmd.Flags().BoolVarP(&interactiveFlag, "interactive", "i", false, "Launch interactive trace viewer")
